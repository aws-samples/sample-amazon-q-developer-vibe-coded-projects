...
# Script for first-time deployment of the Bedrock Agents Proxy application
...
# Step 7: Deploy the Bedrock Agent
echo "=== Deploying Bedrock Agent ==="
npm run deploy:agent

# Step 8: Update environment variables with Bedrock Agent settings
echo "=== Updating environment variables with Bedrock Agent settings ==="
npm run update-env

# Try to get the CloudFront URL from the stack outputs, but don't fail if it's not available
CLOUDFRONT_URL=$(aws cloudformation describe-stacks --stack-name SmartTodoStack --query "Stacks[0].Outputs[?OutputKey=='WebsiteUrl'].OutputValue" --output text 2>/dev/null || echo "Check CloudFormation console for URL")
...
echo -e "\033[1;33m • Name:    \033[0m Bedrock Agents Proxy Application"
echo -e "\033[1;33m • Status:  \033[0m Ready to use"
echo -e "\033[1;33m • URL:     \033[0m ${CLOUDFRONT_URL}"
echo ""
echo -e "\033[1;36m NEXT STEPS \033[0m"
echo -e "\033[1;36m ───────────────────────────────────────────────────────────────\033[0m"
echo -e "\033[0m 1. Access the application using the CloudFront URL from the CloudFormation console"
echo -e "\033[0m 2. Sign up for a new account"
echo -e "\033[0m 3. Try out the chat functionality with Bedrock Agent integration"
echo ""
