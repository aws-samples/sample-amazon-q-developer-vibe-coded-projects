...
export abstract class BaseRepository<T extends Record<string, any>> {
  ...
  async get(pkValue: string, skValue: string): Promise<T | null> {
    logger.debug(`Getting item from DynamoDB table ${this.tableName} with PK: ${pkValue}, SK: ${skValue}`);
  ...
  async update(pkValue: string, skValue: string, updates: Record<string, any>): Promise<T> {
    ...
    logger.debug(`Updating item in DynamoDB table ${this.tableName} with PK: ${pkValue}, SK: ${skValue}, expressions: ${updateExpressions.join(', ')}`);
  ...
  async delete(pkValue: string, skValue: string): Promise<void> {
    logger.debug(`Deleting item from DynamoDB table ${this.tableName} with PK: ${pkValue}, SK: ${skValue}`);
