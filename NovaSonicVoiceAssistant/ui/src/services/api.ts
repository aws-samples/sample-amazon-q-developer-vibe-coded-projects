...
export const todoApi = {
  ...
  getNotes: async (todoId: string) => {
    const response = await api.get(`/todo/${todoId}/notes`);
  ...
  addNote: async (todoId: string, noteData: { content: string }) => {
    const response = await api.post(`/todo/${todoId}/notes`, noteData);
    return response.data;
  },
  
  deleteNote: async (todoId: string, noteId: string) => {
    const response = await api.delete(`/todo/${todoId}/notes/${noteId}`);
    return response.data;
  }
};
